...
data "azurerm_subnet" "SP_IT_Subnet" {
...
# Create SP-IT Route Table
resource "azurerm_route_table" "SP_IT_Route_Table" {
  name                       = "${local.variable.prefix}-${local.variable.sp_it_route_table}"
  location                   = local.variable.location
  resource_group_name        = "${local.variable.prefix}-${local.variable.sp_it_common_rg}"

  dynamic "route" {
    for_each                 = local.variable.sp_it_routes
    content {
...
# For associate sp it common subnet to route table
resource "azurerm_subnet_route_table_association" "PaaS_SP_IT_Subnet" {
  subnet_id                  = "${data.azurerm_subnet.SP_IT_Subnet.id}"
  route_table_id             = azurerm_route_table.SP_IT_Route_Table.id
}