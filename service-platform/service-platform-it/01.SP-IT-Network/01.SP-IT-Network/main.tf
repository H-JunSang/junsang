# Create SP_IT_Common resource group
resource "azurerm_resource_group" "SP_IT_Common_rg" {
...
# Create SP_IT_Common Virtual Network 
resource "azurerm_virtual_network" "SP_IT_Common_VNET" {
  name                = "${local.variable.prefix}-${local.variable.sp_it_common_vnet}"
  location            = azurerm_resource_group.SP_IT_Common_rg.location
  resource_group_name = azurerm_resource_group.SP_IT_Common_rg.name
  address_space       = [local.variable.sp_it_common_address_space]
  dynamic "subnet" {
    for_each = local.variable.sp_it_common_subnets
    content {
