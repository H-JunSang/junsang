...
# SP-Prod Mysql Subnet
resource "azurerm_subnet" "PaaS_SP_Prod_Mysql_Subnet" {
  address_prefixes     = local.variable.sp_prod_mysql_subnet_prefixs
  name                 = "${local.variable.prefix}-${local.variable.sp_prod_mysql_subnet_name}"
  resource_group_name  = "${data.azurerm_resource_group.SP_Prod_RG.name}"
...
# Create SP-Prod Mysql 서브넷 보안 그룹
resource "azurerm_network_security_group" "SP_Prod_MySql_Nsg" {
  name                = "${local.variable.prefix}-${local.variable.sp_prod_mysql_nsg_name}"
  ...
  dynamic "security_rule" {
    for_each = local.sp_prod_mysql_security_rule # From provider.tf variable
    content {
...
resource "azurerm_subnet_network_security_group_association" "SP_Prod_MySql_Associate" {
  subnet_id                 = azurerm_subnet.PaaS_SP_Prod_Mysql_Subnet.id 
  network_security_group_id = azurerm_network_security_group.SP_Prod_MySql_Nsg.id 
}
# Create Mysql Private DNS zone
resource "azurerm_private_dns_zone" "SP_Prod_MySql_Zone" {
  name                = local.variable.sp_prod_mysql_private_dns_zone
  resource_group_name = "${data.azurerm_resource_group.SP_Prod_RG.name}"
}
# Create Mysql Private DNS zone Virtual Network Links
resource "azurerm_private_dns_zone_virtual_network_link" "SP_Prod_vNet_Link" {
  name                  = "${local.variable.prefix}-${local.variable.sp_prod_mysql_private_link_name}"
  private_dns_zone_name = azurerm_private_dns_zone.SP_Prod_MySql_Zone.name
  resource_group_name   = "${data.azurerm_resource_group.SP_Prod_RG.name}"
...
resource "azurerm_mysql_flexible_server" "SP_Prod_MySql" {
  location                     = local.variable.location
  name                         = local.variable.sp_prod_mysql_name
  resource_group_name          = "${data.azurerm_resource_group.SP_Prod_RG.name}"
  ...
  delegated_subnet_id          = azurerm_subnet.PaaS_SP_Prod_Mysql_Subnet.id
  geo_redundant_backup_enabled = local.variable.geo_redundant_backup_enabled
  private_dns_zone_id          = azurerm_private_dns_zone.SP_Prod_MySql_Zone.id
  ...
  depends_on = [azurerm_private_dns_zone_virtual_network_link.SP_Prod_vNet_Link]
