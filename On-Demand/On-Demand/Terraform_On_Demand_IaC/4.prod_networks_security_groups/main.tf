...
# Create Prod IT Common 서브넷 정보
data "azurerm_subnet" "Prod_It_Common_Subnet" {
  name                 = "${local.variable.prefix}-${local.variable.prod_it_common_subnet}"
  resource_group_name  = "${local.variable.prefix}-${local.variable.prodrg}"
  virtual_network_name = "${local.variable.prefix}-${local.variable.prod_vnet}"
}
# Create Prod IT Inference 서브넷 정보
data "azurerm_subnet" "Prod_It_Inference_Subnet" {
...
output "Prod_It_Common_Subnet_id" {
  value = data.azurerm_subnet.Prod_It_Common_Subnet.id
}
# Create Prod IT Inference ID 추출
output "Prod_It_Inference_Subnet_id" {
  value = data.azurerm_subnet.Prod_It_Inference_Subnet.id
}
...
resource "azurerm_network_security_group" "Prod_It_Common_Nsg" {
  name                = "${local.variable.prefix}-${local.variable.prod_it_common_security_group}"
  location            = local.variable.location
  resource_group_name = "${local.variable.prefix}-${local.variable.prodrg}"
  tags = {
    Creator     = local.variable.prod_tag_creator
...
resource "azurerm_subnet_network_security_group_association" "Prod_It_Common_Associate" {
  subnet_id                 = data.azurerm_subnet.Prod_It_Common_Subnet.id # From Prod IT Common 서브넷 ID Output Value
  network_security_group_id = azurerm_network_security_group.Prod_It_Common_Nsg.id # NSG ID From Prod IT Common 보안 그룹 리소스
}
# Create Prod IT Inference 서브넷 보안 그룹
resource "azurerm_network_security_group" "Prod_It_Inference_Nsg" {
  name                = "${local.variable.prefix}-${local.variable.prod_it_inference_security_group}"
  location            = local.variable.location
  resource_group_name = "${local.variable.prefix}-${local.variable.prodrg}"
  tags = {
    Creator     = local.variable.prod_tag_creator
...
resource "azurerm_subnet_network_security_group_association" "Prod_It_Inference_Associate" {
  subnet_id                 = data.azurerm_subnet.Prod_It_Inference_Subnet.id # From IT Inference 서브넷 ID Output Value
  network_security_group_id = azurerm_network_security_group.Prod_It_Inference_Nsg.id # NSG ID From IT Inference 보안 그룹 리소스
}