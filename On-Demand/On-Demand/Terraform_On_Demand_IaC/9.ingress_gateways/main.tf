...
data "azurerm_subnet" "Shared_L7_Subnet" {
...
# Prod L7 로드밸런서 서브넷 정보
data "azurerm_subnet" "Prod_L7_Subnet" {
...
# Create Shared K8S L7 로드밸런서 Public IP
resource "azurerm_public_ip" "Shared_L7_PIP" {
  name                = "${local.variable.prefix}-${local.variable.shared_k8s_l7_PIP}"
  resource_group_name = "${data.azurerm_resource_group.Shared_RG.name}"
...
# Create Shared K8S L7 로드밸런서
resource "azurerm_application_gateway" "Shared_K8s_L7" {
  name                = "${local.variable.prefix}-${local.variable.shared_k8s_l7_gateway_name}"
  resource_group_name = "${data.azurerm_resource_group.Shared_RG.name}"
  location            = local.variable.location
  enable_http2        = local.variable.enable_http2

# Shared K8S Setting SKU
  sku {
    name              = local.variable.sku_tier
    tier              = local.variable.sku_tier
  }

# Shared K8S Setting 오토스케일링 설정
  autoscale_configuration {
    min_capacity      = local.variable.min_capacity
    max_capacity      = local.variable.max_capacity
  }

# Shared K8S L7 로드밸런서 사설 서브넷 할당
  gateway_ip_configuration {
    name              = local.variable.shared_k8s_l7_ip_name
    subnet_id         = "${data.azurerm_subnet.Shared_L7_Subnet.id}"
  }

# Shared K8S L7 로드밸런서 프론트엔드 Port 설정
  frontend_port {
    name              = local.variable.frontend_port_name
    port              = local.variable.frontend_port_name
  }

# Shared K8S L7 로드 밸런서 프론트엔트 Public IP 할당
  frontend_ip_configuration {
    name                 = local.variable.frontend_ip_configuration_name
    public_ip_address_id = azurerm_public_ip.Shared_L7_PIP.id
  }

# Shared K8S L7 로드 밸런서 백엔드 풀 설정
  backend_address_pool {
    name                 = local.variable.backend_address_pool_name
  }
# Shared K8S L7 로드 밸런서 백엔드 HTTP 설정
...
###################################################################################
# Create Prod K8S L7 로드밸런서 Public IP
resource "azurerm_public_ip" "Prod_L7_PIP" {
  name                = "${local.variable.prefix}-${local.variable.prod_k8s_l7_PIP}"
  resource_group_name = "${data.azurerm_resource_group.Prod_RG.name}"
  location            = local.variable.location
  allocation_method   = local.variable.allocate_public_ip
  sku                 = local.variable.public_ip_sku
...
# Create Prod K8S L7 로드밸런서
resource "azurerm_application_gateway" "Prod_K8s_L7" {
  name                = "${local.variable.prefix}-${local.variable.prod_k8s_l7_gateway_name}"
  resource_group_name = "${data.azurerm_resource_group.Prod_RG.name}"
  location            = local.variable.location
  enable_http2        = local.variable.enable_http2

# Prod K8S Setting SKU
  sku {
    name              = local.variable.sku_tier
    tier              = local.variable.sku_tier
  }

# Prod K8S Setting 오토스케일링 설정
  autoscale_configuration {
    min_capacity      = local.variable.min_capacity
    max_capacity      = local.variable.max_capacity
  }

# Prod K8S L7 로드밸런서 사설 서브넷 할당
  gateway_ip_configuration {
    name              = local.variable.prod_k8s_l7_ip_name
    subnet_id         = "${data.azurerm_subnet.Prod_L7_Subnet.id}"
  }

# Prod K8S L7 로드밸런서 프론트엔드 Port 설정
  frontend_port {
    name              = local.variable.frontend_port_name
    port              = local.variable.frontend_port_name
  }

# Prod K8S L7 로드 밸런서 프론트엔트 Public IP 할당
  frontend_ip_configuration {
    name                 = local.variable.frontend_ip_configuration_name
    public_ip_address_id = azurerm_public_ip.Prod_L7_PIP.id
  }

# Prod K8S L7 로드 밸런서 백엔드 풀 설정
  backend_address_pool {
    name                 = local.variable.backend_address_pool_name
  }

# Prod K8S L7 로드 밸런서 백엔드 HTTP 설정
  backend_http_settings {
    name                  = local.variable.http_setting_name
    cookie_based_affinity = local.variable.cookie_affinity
    path                  = local.variable.backend_path
